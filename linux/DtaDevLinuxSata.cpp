...
uint8_t DtaDevLinuxSata::sendCmd (ATACOMMAND cmd, uint8_t protocol,
...
void DtaDevLinuxSata::identify (OPAL_DiskInfo& disk_info) {
   ...
   cdb[9] = 0xec; // IF_SEND/IF_RECV
    ...
    std::ifstream kopts;
    kopts.open("/sys/module/libata/parameters/allow_tpm", std::ios::in);
    if (!kopts) {
...
/** Send an ioctl to the device using pass through. */
uint8_t DtaDevLinuxSata::sendCmd_SAS (ATACOMMAND cmd, uint8_t protocol,
    uint16_t comID, void *buffer, uint32_t bufferlen)
{
    sg_io_hdr_t sg;
    uint8_t sense   [32]; // how big should this be??
...
static void safecopy (uint8_t *dst, size_t dstsize, uint8_t *src, size_t srcsize) {
    const size_t size = std::min(dstsize, srcsize);
    if (size > 0) {
        memcpy(dst, src, size);
    }
    if (size < dstsize) {
...
void DtaDevLinuxSata::identify_SAS (OPAL_DiskInfo *disk_info) {
    ...
    safecopy(disk_info->serialNum, sizeof(disk_info->serialNum),
        resp->m_T10VendorId, sizeof(resp->m_T10VendorId));
    safecopy(disk_info->firmwareRev, sizeof(disk_info->firmwareRev),
        resp->m_ProductRevisionLevel, sizeof(resp->m_ProductRevisionLevel));
    safecopy(disk_info->modelNum, sizeof(disk_info->modelNum),
        resp->m_ProductId, sizeof(resp->m_ProductId));
