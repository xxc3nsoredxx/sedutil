...
public:
     ...
     */
    uint8_t getTable (std::vector<uint8_t> table, uint16_t startcol, uint16_t endcol);
     ...
     */
    uint8_t setTable (std::vector<uint8_t> table, OPAL_TOKEN name,
     ...
     * @param value data to be stored the the column 
     */
    uint8_t setTable (std::vector<uint8_t> table, OPAL_TOKEN name, OPAL_TOKEN value);
    ...
    uint8_t enableUser (char *password, char *userid,
        OPAL_TOKEN status = OPAL_TOKEN::OPAL_TRUE);
     ...
     */
    uint8_t rekeyLockingRange_SUM (std::vector<uint8_t> LR,
     ...
     */
    uint8_t setup_SUM (uint8_t lockingrange, uint64_t start, uint64_t length,
     ...
     */
    uint8_t rawCmd (char *sp, char *auth, char *pass, char *invoker,
        char *method, char *plist);
protected:
