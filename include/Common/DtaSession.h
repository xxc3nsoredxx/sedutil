...
public:
     ...
     */    
#ifdef MULTISTART
    uint8_t unistart (OPAL_UID SP, char *HostChallenge, std::vector<uint8_t> SignAuthority);
#endif
     ...
     * @param SignAuthority the Signing authority (in a simple session this is the user)
     */
    uint8_t start (OPAL_UID SP, char *HostChallenge, std::vector<uint8_t> SignAuthority);
    /** Authenticate an already started session 
     * @param Authority the authority to authenticate
     * @param Challenge the password
     */
    uint8_t authenticate (std::vector<uint8_t> Authority, char *Challenge);
