...
void DtaDev::discovery0 () {
    ...
    epos    = (uint8_t *)d0Response;
    cpos    = (uint8_t *)d0Response;
    ...
    do {
        ...
        default:
            if (0xbfff < (SWAP16(body->TPer.featureCode))) {
                ...
                disk_info.Unknown += 1;
                LOG(D) << "Unknown Feature in Discovery 0 response " << std::hex
                       << SWAP16(body->TPer.featureCode) << std::dec;
...
void DtaDev::puke () {
    ...
    std::cout << std::endl << dev
              << (disk_info.devType == DEVICE_TYPE_ATA  ? " ATA "  :
                  ...
                  " OTHER ");
    std::cout << disk_info.modelNum << " " << disk_info.firmwareRev << " "
              << disk_info.serialNum << std::endl;
    /* TPer */
    if (disk_info.TPer) {
        std::cout << "TPer function (" << HEXON(4) << FC_TPER << HEXOFF << ")"
                  << std::endl;
        std::cout << "    ACKNAK = " << (disk_info.TPer_ACKNACK ? "Y, " : "N, ")
    ...
    if (disk_info.Locking) {
        std::cout << "Locking function (" << HEXON(4) << FC_LOCKING << HEXOFF << ")"
                  << std::endl;
        std::cout << "    Locked = " << (disk_info.Locking_locked ? "Y, " : "N, ")
                  << "LockingEnabled = " << (disk_info.Locking_lockingEnabled ? "Y, " : "N, ")
                  << "LockingSupported = " << (disk_info.Locking_lockingSupported ? "Y, " : "N, ");
        std::cout << "MBRDone = " << (disk_info.Locking_MBRDone ? "Y, " : "N, ")
                  << "MBREnabled = " << (disk_info.Locking_MBREnabled ? "Y, " : "N, ")
                  << "MediaEncrypt = " << (disk_info.Locking_mediaEncrypt ? "Y" : "N")
                  << std::endl;
    }
    if (disk_info.Geometry) {
        std::cout << "Geometry function (" << HEXON(4) << FC_GEOMETRY << HEXOFF << ")"
                  << std::endl;
        std::cout << "    Align = " << (disk_info.Geometry_align ? "Y, " : "N, ")
    ...
    if (disk_info.Enterprise) {
        std::cout << "Enterprise function (" << HEXON(4) << FC_ENTERPRISE << HEXOFF << ")"
                  << std::endl;
        std::cout << "    Range crossing = " << (disk_info.Enterprise_rangeCrossing ? "Y, " : "N, ")
                  << "Base comID = " << HEXON(4) << disk_info.Enterprise_basecomID
                  << ", comIDs = " << disk_info.Enterprise_numcomID << HEXOFF
                  << std::endl;
    }
    if (disk_info.OPAL10) {
        std::cout << "Opal V1.0 function (" << HEXON(4) << FC_OPALV100 << HEXOFF << ")"
                  << std::endl;
        std::cout << "Base comID = " << HEXON(4) << disk_info.OPAL10_basecomID << HEXOFF
                  << ", comIDs = " << disk_info.OPAL10_numcomIDs
                  << std::endl;
    }
    if (disk_info.SingleUser) {
        std::cout << "SingleUser function (" << HEXON(4) << FC_SINGLEUSER << HEXOFF << ")"
                  << std::endl;
        std::cout << "    ALL = " << (disk_info.SingleUser_all ? "Y, " : "N, ")
    ...
    if (disk_info.DataStore) {
        std::cout << "DataStore function (" << HEXON(4) << FC_DATASTORE << HEXOFF << ")"
                  << std::endl;
        std::cout << "    Max Tables = " << disk_info.DataStore_maxTables
    ...
    if (disk_info.OPAL20) {
        std::cout << "OPAL 2.0 function (" << HEXON(4) << FC_OPALV200 << ")" << HEXOFF
                  << std::endl;
        std::cout << "    Base comID = " << HEXON(4) << disk_info.OPAL20_basecomID << HEXOFF;
        std::cout << ", Initial PIN = " << HEXON(2) << disk_info.OPAL20_initialPIN << HEXOFF;
        std::cout << ", Reverted PIN = " << HEXON(2) << disk_info.OPAL20_revertedPIN << HEXOFF;
        std::cout << ", comIDs = " << disk_info.OPAL20_numcomIDs;
        std::cout << std::endl;
        std::cout << "    Locking Admins = " << disk_info.OPAL20_numAdmins;
        std::cout << ", Locking Users = " << disk_info.OPAL20_numUsers;
        std::cout << ", Range Crossing = " << (disk_info.OPAL20_rangeCrossing ? "Y" : "N");
        std::cout << std::endl;
    }
    if (disk_info.Unknown) {
        std::cout << "**** " << (uint16_t)disk_info.Unknown
                  << " **** Unknown function codes IGNORED " << std::endl;
    }
}
