...
uint8_t DtaDevOpal::setup_SUM (uint8_t lockingrange, uint64_t start,
    uint64_t length, char *Admin1Password, char *password)
{
    ...
    if (start != lrStatus.start || length != lrStatus.size) {
        LOG(D1) << "Incorrect Locking Range " << lockingrange
                << " start/size. Attempting to correct...";
        if ((lastRC = setupLockingRange_SUM(lockingrange, start, length, defaultPW)) != 0) {
            LOG(E) << "Setup_SUM failed - unable to setup locking range "
                   << lockingrange << "(" << start << "," << length << ")";
            return lastRC;
...
DtaDevOpal::lrStatus_t DtaDevOpal::getLockingRange_status (uint8_t lockingrange,
    char *password)
{
    uint8_t lastRC;
    lrStatus_t lrStatus;
...
uint8_t DtaDevOpal::rekeyLockingRange_SUM (std::vector<uint8_t> LR,
    std::vector<uint8_t> UID, char *password)
{
...
uint8_t DtaDevOpal::revertLockingSP (char *password, uint8_t keep) {
    ...
    uint8_t lastRC;
    std::vector<uint8_t> keepGlobalLocking;
...
uint8_t DtaDevOpal::eraseLockingRange (uint8_t lockingrange, char *password) {
    ...
    if (password == NULL) {
        LOG(D4) << "Referencing formal parameters " << lockingrange;
    }
...
uint8_t DtaDevOpal::getAuth4User (char *userid, uint8_t uidorcpin,
...
uint8_t DtaDevOpal::setNewPassword_SUM (char *password, char *userid,
    char *newpassword)
{
...
uint8_t DtaDevOpal::setMBREnable (uint8_t mbrstate, char *Admin1Password) {
    ...
    if (mbrstate) {
        if ((lastRC = setLockingSPvalue(OPAL_UID::OPAL_MBRCONTROL,
            OPAL_TOKEN::MBRENABLE, OPAL_TOKEN::OPAL_TRUE, Admin1Password, NULL)) != 0)
        {
        ...
        if ((lastRC = setLockingSPvalue(OPAL_UID::OPAL_MBRCONTROL,
            OPAL_TOKEN::MBRENABLE, OPAL_TOKEN::OPAL_FALSE, Admin1Password, NULL)) != 0)
        {
...
uint8_t DtaDevOpal::setMBRDone (uint8_t mbrstate, char *Admin1Password) {
    ...
    if (mbrstate) {
        if ((lastRC = setLockingSPvalue(OPAL_UID::OPAL_MBRCONTROL,
            OPAL_TOKEN::MBRDONE, OPAL_TOKEN::OPAL_TRUE, Admin1Password, NULL)) != 0)
        {
        ...
        if ((lastRC = setLockingSPvalue(OPAL_UID::OPAL_MBRCONTROL,
            OPAL_TOKEN::MBRDONE, OPAL_TOKEN::OPAL_FALSE, Admin1Password, NULL)) != 0)
        {
    ...
    char *Admin1Password)
{
    ...
    if ((lastRC = session->start(OPAL_UID::OPAL_LOCKINGSP_UID, Admin1Password,
        OPAL_UID::OPAL_ADMIN1_UID)) != 0)
    {
        delete session;
        return lastRC;
    ...
    char *password)
{
    ...
    auth.push_back(lockingrange+1);
    if ((lastRC = session->start(OPAL_UID::OPAL_LOCKINGSP_UID,
        password, auth)) != 0)
    {
        LOG(E) << "Error starting session. Did you provide the correct user password? (GlobalRange = User1; Range1 = User2, etc.)";
        delete session;
...
uint8_t DtaDevOpal::loadPBA (char *password, char *filename) {
    ...
    uint32_t filepos = 0;
    uint32_t eofpos;
    std::ifstream pbafile;
    (MAX_BUFFER_LENGTH > tperMaxPacket) ? blockSize = tperMaxPacket : blockSize = MAX_BUFFER_LENGTH;
    if (blockSize > (tperMaxToken - 4)) {
        blockSize = tperMaxToken - 4;
    }
    std::vector<uint8_t> buffer;
    std::vector<uint8_t> lengthtoken;
    blockSize -= sizeof(OPALHeader) + 50;  // packet overhead
    buffer.resize(blockSize);
    pbafile.open(filename, std::ios::in | std::ios::binary);
    if (!pbafile) {
...
uint8_t DtaDevOpal::activateLockingSP (char *password) {
    ...
    if ((lastRC = session->start(OPAL_UID::OPAL_ADMINSP_UID, password,
        OPAL_UID::OPAL_SID_UID)) != 0)
    {
        delete cmd;
        delete session;
        return lastRC;
...
uint8_t DtaDevOpal::activateLockingSP_SUM (uint8_t lockingrange, char *password) {
    ...
    LOG(I) << "Locking SP Activate Complete for single User" << (lockingrange + 1)
           << " on locking range " << (int)lockingrange;
...
uint8_t DtaDevOpal::printDefaultPassword () {
    ...
    std::string defaultPassword = response.getString(4);
...
uint8_t DtaDevOpal::setTable (std::vector<uint8_t> table, OPAL_TOKEN name,
    OPAL_TOKEN value)
{
...
uint8_t DtaDevOpal::getTable (std::vector<uint8_t> table, uint16_t startcol, 
    uint16_t endcol)
{
...
uint8_t DtaDevOpal::exec (DtaCommand *cmd, DtaResponse& resp, uint8_t protocol) {
    ...
    if ((lastRC = sendCmd(IF_SEND, protocol, comID(), cmd->getCmdBuffer(),
        cmd->outputBufferSize())) != 0)
    {
    ...
    do {
        ...
        lastRC = sendCmd(IF_RECV, protocol, comID(), cmd->getRespBuffer(),
            MIN_BUFFER_LENGTH);
...
uint8_t DtaDevOpal::properties () {
    ...
    DtaCommand *props = new DtaCommand(OPAL_UID::OPAL_SMUID_UID,
        OPAL_METHOD::PROPERTIES);
    if (NULL == props) {
        LOG(E) << "Unable to create command object ";
        delete session;
        return DTAERROR_OBJECT_CREATE_FAILED;
...
void DtaDevOpal::puke () {
    ...
    if (disk_info.Properties) {
        std::cout << std::endl << "TPer Properties: ";
        for (uint32_t i = 0; i < propertiesResponse.getTokenCount(); i++) {
            if (OPAL_TOKEN::STARTNAME == propertiesResponse.tokenIs(i)) {
                if (OPAL_TOKEN::DTA_TOKENID_BYTESTRING != propertiesResponse.tokenIs(i + 1)) {
                    std::cout << std::endl << "Host Properties: " << std::endl;
                } else {
                    std::cout << "  " << propertiesResponse.getString(i + 1)
...
uint8_t DtaDevOpal::objDump (char *sp, char *auth, char *pass, char *objID) {
    ...
    char *hexinvokingUID, char *hexmethod, char *hexparms)
{
    ...
    std::vector<uint8_t> authority;
    std::vector<uint8_t> object;
    std::vector<uint8_t> invokingUID;
    std::vector<uint8_t> method;
    std::vector<uint8_t> parms;
    uint8_t work;
    ...
    for (uint32_t i = 0; i < 16; i += 2) {
        work = hexinvokingUID[i] & 0x40
             ? 16 * ((hexinvokingUID[i] & 0xf) + 9)
             : 16 * (hexinvokingUID[i] & 0x0f);
        work += hexinvokingUID[i + 1] & 0x40
             ? (hexinvokingUID[i + 1] & 0xf) + 9
             : hexinvokingUID[i + 1] & 0x0f;
    ...
    for (uint32_t i = 0; i < 16; i += 2) {
        work = hexmethod[i] & 0x40
             ? 16 * ((hexmethod[i] & 0xf) + 9)
             : 16 * (hexmethod[i] & 0x0f);
        work += hexmethod[i + 1] & 0x40
             ? (hexmethod[i + 1] & 0xf) + 9
             : hexmethod[i + 1] & 0x0f;
    ...
    for (uint32_t i = 0; i < strnlen(hexparms, 1024); i += 2) {
        work = hexparms[i] & 0x40
             ? 16 * ((hexparms[i] & 0xf) + 9)
             : 16 * (hexparms[i] & 0x0f);
        work += hexparms[i + 1] & 0x40
             ? (hexparms[i + 1] & 0xf) + 9
             : hexparms[i + 1] & 0x0f;
