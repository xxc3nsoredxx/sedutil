...
void DtaToken::parse (uint8_t *buf, uint32_t buflen) {
    ...
    if (atom.TinyAtom.indicator == atom.TinyAtom.INDICATOR) {
        ...
        m_DataLength    = uint32_t(atom.MediumAtom.length_msb) << 8
                        | uint32_t(atom.MediumAtom.length_lsb);
        ...
        m_DataLength    = (uint32_t(atom.LongAtom.length[0]) << 8
                           | uint32_t(atom.LongAtom.length[1])) << 8
                        | uint32_t(atom.LongAtom.length[2]);
...
int DtaToken::print (FILE *stream, uint32_t buflen) {
    ...
    unsigned int len = std::min(m_HeaderLength, buflen);
    ret += fprintf(stream, "(");
    for (unsigned int n = 0; n < len; n++) {
        ret += fprintf(stream, " %2.2X", uint32_t(m_token[n]) & 0xff);
    }
    ret += fprintf(stream, " )");
    buflen -= len;

    // print out atom value
    len = std::min(m_DataLength, buflen);
    if (m_TokenType == TOKEN) {
        ...
        // byte string
        for (unsigned int n = 0; n < len; n++) {
            ret += fprintf(stream, " %2.2X", uint32_t(m_data[n]) & 0xff);
        }
...
int DtaToken::printUID (FILE *stream, uint8_t buf [8]) {
    ...
    for (int n = 0; n < len1; n++) {
        if (!memcmp(list1[n].UID, buf, sizeof(list1[n].UID)))
            return fprintf(stream, "%s", list1[n].name);
    }
    ...
    for (int n = 0; n < len2; n++) {
        if (!memcmp(list2[n].UID, buf2, sizeof(list2[n].UID)))
            return fprintf(stream, list2[n].name, band);
    }
...
uint8_t DtaAnnotatedDump (ATACOMMAND cmd, void *buffer, uint32_t bufferlen) {
    ...
    while(p < q) {
        ...
        if (p + token.m_TokenLength > q)
            fprintf(stream, "(token truncated)");
